...
## 开发与扩展建议 Development & Extension

- **增加难度等级与题目类型前端配置**  
  - 在教师模式表单中添加 `difficulty_level` 与 `problem_types` 选择控件。  
  - 将这些参数一并发送给后端，配合题库标签实现更精细控制。  

- **接入本地或校内后端服务**  
  - 将后端部署在校内服务器，前端通过环境变量配置 API Base URL。  

- **导出 PDF / 打印作业**  
  - 为教师提供“一键生成 PDF 作业”或“导出为题目列表”的功能，方便线下发放。  

---

如需进一步优化 README 的结构或语言风格，欢迎继续告诉我你的具体需求。
